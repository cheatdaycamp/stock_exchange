class CompanyCard {
  constructor(utils, symbol, row, API) {
    this.API = API;
  ...
  getCompanyfromLocalStorage = async () => {
    console.log("here");
    let url = `https://financialmodelingprep.com/api/v3/profile/${this.symbol}?apikey=${this.API}`;
    console.log(url);
    let tryGrabFromStorage = this.utils.getCompaniesfromLocalStorage(
      "COMPANIES"
    );
                ...
                <div class="m-3 card col-xs-11 col-sm-11 col-md-11 col-lg-5 col-xl-4 p-3 d-flex flex-column align-items-stretch">
                    <div class = 'container-fluid d-flex flex-column flex-grow-1 h-100'>
                        ...
                        <div class = 'row mt-3'>
                            ...
                            <span class=${this.utils.getColor(
  ...
  getDataChart = async () => {
    let url = `https://financialmodelingprep.com/api/v3/historical-price-full/${this.company.symbol}?serietype=line&apikey=${this.API}`;
